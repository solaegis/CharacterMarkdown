...
# CharacterMarkdown - ESOUI API upload helper
# Uses the official ESOUI field name "compatible" (not "compatibility").
...
if [ -n "$COMPATIBLE_VERSION" ] && ! echo "$COMPATIBLE_VERSION" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+$'; then
    echo "Compatible version must be MAJOR.MINOR.PATCH: $COMPATIBLE_VERSION" >&2
    exit 1
fi
...
if [ -n "$COMPATIBLE_VERSION" ] && command -v jq >/dev/null 2>&1; then
    if COMPATIBLE_LIST="$(curl --silent --fail -H "x-api-token: ${API_KEY}" \
        ...
        if [ -n "$RESOLVED" ]; then
            COMPATIBLE="$RESOLVED"
            echo "Resolved ESOUI compatible value: $COMPATIBLE"
        else
            echo "No compatible.json match for $COMPATIBLE_VERSION; using version string"
        fi
    else
        echo "Could not fetch compatible.json; using version string $COMPATIBLE_VERSION"
    fi
fi
...
if [ -f "$CHANGELOG_FILE" ]; then
    CURL_ARGS+=(-F "changelog=<${CHANGELOG_FILE}")
fi

if [ -f "$DESCRIPTION_FILE" ]; then
    CURL_ARGS+=(-F "description=<${DESCRIPTION_FILE}")
fi

if [ -n "$COMPATIBLE" ]; then
    CURL_ARGS+=(-F "compatible=${COMPATIBLE}")
fi

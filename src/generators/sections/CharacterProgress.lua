        ...
        for _, skillType in ipairs(morphsData.skillTypes) do
            if #skillType.skillLines > 0 then
                ...
                table_insert(
                    parts,
                    string_format(
                        "### %s %s (%d abilities with morph choices)\n\n",
                        emoji,
                        skillType.name,
                        typeAbilityCount
                    )
                ...
                for _, line in ipairs(skillType.skillLines) do
                    ...
                    for _, ability in ipairs(line.abilities) do
                        ...
                        local link = (CreateSkillLink and CreateSkillLink(morphName))
                            or string_format(
                                "[%s](https://en.uesp.net/wiki/Online:%s)",
                                morphName,
                                morphName:gsub(" ", "_")
                            )
                        ...
                        if ability.currentMorph > 0 then
                            ...
                            for _, m in ipairs(ability.morphs) do
                                if m.selected then
                                    local mLink = (CreateSkillLink and CreateSkillLink(m.name))
                                        or string_format(
                                            "[%s](https://en.uesp.net/wiki/Online:%s)",
                                            m.name,
                                            m.name:gsub(" ", "_")
        ...
        for _, typeName in ipairs(typeOrder) do
            ...
            if lines and #lines > 0 then
                ...
                for _, line in ipairs(lines) do
                    ...
                    local link = (CreateSkillLineLink and CreateSkillLineLink(line.name))
                        or string_format(
                            "**[%s](https://en.uesp.net/wiki/Online:%s)**",
                            line.name,
                            line.name:gsub(" ", "_")
                        ...
                        if hasPurchasedPassives then
                            ...
                            for _, passive in ipairs(line.passives) do
                                if passive and type(passive) == "table" and passive.name then
                                    ...
                                    local pLink = (CreateSkillLink and CreateSkillLink(passiveName))
                                        or string_format(
                                            "[%s](https://en.uesp.net/wiki/Online:%s)",
                                            passiveName,
                                            passiveName:gsub(" ", "_")

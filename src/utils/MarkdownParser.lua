...
local function SplitSection(section, maxSize)
    ...
    if #subsections > 1 then
        CM.DebugPrint("CHUNKING", string.format("Splitting section into %d subsections", #subsections))
    ...
    if #tables > 1 then
        CM.DebugPrint("CHUNKING", string.format("Splitting section at %d table boundaries", #tables))

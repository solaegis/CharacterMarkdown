...
local function BuildChunks(sections, maxSize, options)
    ...
    CM.DebugPrint("CHUNKING", string.format("Building chunks from %d sections (maxSize: %d)", #sections, maxSize))
    ...
    for sectionIndex, section in ipairs(sections) do
        ...
        if CanFitInChunk(currentChunk, sectionContent, maxSize) then
            ...
            CM.DebugPrint(
                "CHUNKING",
                string.format("Added section to current chunk (now %d bytes)", string_len(currentChunk))
            )
        else
            ...
            if currentChunk ~= "" and string_len(currentChunk) >= minChunkSize then
                ...
                CM.DebugPrint(
                    "CHUNKING",
                    string.format("Finalized chunk %d: %d bytes", chunkNum, string_len(currentChunk))
                )
            ...
            if sectionSize > maxSize then
                CM.DebugPrint("CHUNKING", string.format("Section too large (%d > %d), splitting", sectionSize, maxSize))
                ...
                CM.DebugPrint("CHUNKING", string.format("Started new chunk with section (%d bytes)", sectionSize))
            end
        end
    end
    ...
    if currentChunk ~= "" then
        ...
        CM.DebugPrint(
            "CHUNKING",
            string.format("Finalized final chunk %d: %d bytes", chunkNum, string_len(currentChunk))
        )
    end
    ...
    CM.DebugPrint(
        "CHUNKING",
        string.format("Built %d chunks from %d sections (with boundary markers)", #chunks, #sections)
    )

...
local function GetNumericQuality(qualityString)
    ...
    -- ESO standard quality names
    if qualityString == "Trash" then
        return 0
    elseif qualityString == "Normal" then
        return 1
    elseif qualityString == "Fine" then
        return 2
    elseif qualityString == "Superior" then
        return 3
    elseif qualityString == "Epic" then
        return 4
    elseif qualityString == "Legendary" then
        return 5
    -- Backward compatibility with old names
    elseif qualityString == "Magic" then
        return 2
    elseif qualityString == "Arcane" then
        return 3
    elseif qualityString == "Artifact" then
        return 4
    else
        return 0
    end
...
local function AnalyzeEnchantments(equipmentData)
    ...
    for _, item in ipairs(equipmentData.items or {}) do
        ...
        local hasEnchantment = (item.enchantment and item.enchantment ~= false and item.enchantment ~= "")
            or (
                item.enchantCharge and (type(item.enchantCharge) == "number" and item.enchantCharge > 0)
                or (type(item.enchantCharge) == "string" and item.enchantCharge ~= "")
            )
        ...
        if item.name and item.name ~= "-" and hasEnchantment then
            ...
            local stripColors = StripColorCodes or function(text)
                return text
            end
            ...
            local enchantData = {
                ...
                chargePercent = (item.enchantMaxCharge or 0) > 0 and math.floor(
                    (numericCharge / (item.enchantMaxCharge or 1)) * 100
                ) or 0,
            }

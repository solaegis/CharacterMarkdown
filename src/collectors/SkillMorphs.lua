...
local function CollectSkillMorphsData()
        ...
        if not invalidSkillTypes[skillTypeName] then
            ...
            for skillLineIndex = 1, numSkillLines do
                ...
                if success and skillLineName and not invalidSkillLines[skillLineName] then
                    ...
                    local isPlayerClass = not isClassSkill or 
                                        (classSkillLines[playerClass] and 
                                         classSkillLines[playerClass][skillLineName])
                    ...
                    if isPlayerClass then
                        ...
                        for abilityIndex = 1, numAbilities do
                            local success2, abilityName, icon, earnedRank, passive, ultimate, purchased, 
                            ...
                            if success2 and abilityName and not passive and progressionIndex then
                                ...
                                if not processedProgressions[progressionIndex] then
                                    ...
                                    local success3, progName, currentMorph, currentRank = pcall(GetAbilityProgressionInfo, progressionIndex)
                                    
                                    if success3 and progName then
                                        -- Get XP info to check if at morph level
                                        local success4, lastXP, nextXP, currXP, atMorph = pcall(GetAbilityProgressionXPInfo, progressionIndex)
                                        ...
                                        -- Morph 1 (upper morph)
                                        local morph1Success, morph1Id = pcall(GetAbilityProgressionAbilityId, progressionIndex, 1, 1)
                                        ...
                                        -- Morph 2 (lower morph)
                                        local morph2Success, morph2Id = pcall(GetAbilityProgressionAbilityId, progressionIndex, 2, 1)
    ...
    if CM.DebugPrint then
        ...
        for i, skillType in ipairs(morphsData) do
            CM.DebugPrint("SKILL_MORPHS", string.format("  %d. %s (%d skill lines)", i, skillType.name, #skillType.skillLines))
        end
    end

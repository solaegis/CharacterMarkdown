...
function api.GetResearchInfo(craftingType)
    ...
    for l = 1, numLines do
        local success, name, icon, numTraits, timeRequired =
            CM.SafeCallMulti(GetSmithingResearchLineInfo, craftingType, l)
        if name then
            local traits = {}
            -- Check traits
            ...
            for t = 1, numTraitItems do
                local success_trait, traitType, desc, known =
                    CM.SafeCallMulti(GetSmithingResearchLineTraitInfo, craftingType, l, t)
                table.insert(traits, {
                    id = traitType,
                    known = known,
                })
